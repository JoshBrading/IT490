...
app.post("/api/userRequest", async (req, res) => {
  ...
  switch (request) {
    ...
    case RabbitTypes.user.add_game_pack:
      //console.log(req.body)
      // for each in req.body['games']
      for(var i = 0; i < req.body['games'].length; i++){
        const data = {
          type: "add_game_pack",
          username: req.body['username'],
          name: req.body['name']['name'],
          id: req.body['games'][i]['value'],
          game_name: req.body['games'][i]['label']
        };
        //console.log(req.body['games'][i]['label'])
        Rabbit.sendRequest(data).then((response) => {
          console.log(response);
        });
      }
...
app.post("/api/gameRequest", async (req, res) => {
  ...
  //(request, RabbitTypes.game.get_all_steam_games);
  switch (request) {
    case RabbitTypes.game.get_all_steam_games:
      ...
      break;
    case RabbitTypes.game.get_steam_game_short:
      Rabbit.sendRequest({ type: RabbitTypes.game.get_all_steam_games }).then(
        (response) => {
          //console.log(JSON.parse(response));
          const game_details = JSON.parse(response);
          const games = [];
          for( const game in game_details ){
            games.push({name: game_details[game][1], id: game_details[game][0]});
          }
          //console.log(games)
